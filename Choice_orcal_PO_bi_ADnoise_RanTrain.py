...
def q_learning(num_episodes, discount_factor=0.9, alpha=0.1, ordinal_error = 0.3, calculate_error = 0.5,
    ...
    for i_episode in range(num_episodes):
        ...
        value_ha, value_va = np.random.multivariate_normal(mean_h, cov, num_size).T
        value_hb, value_vb = np.random.multivariate_normal(mean_v, cov, num_size).T
        value_hdh, value_vdh = np.random.multivariate_normal(mean_dh, cov, num_size).T
        value_hdv, value_vdv = np.random.multivariate_normal(mean_dv, cov, num_size).T
        '''
...
stats = q_learning(num_episodes = 1500)
